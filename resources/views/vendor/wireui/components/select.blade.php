<div {{ $attributes->only(['class', 'wire:key'])->class('relative w-full') }} x-data="wireui_select({
    ...
    <div class="relative">
        ...
        <x-dynamic-component :component="WireUi::component('input')"
            class="!dark:text-transparent cursor-pointer overflow-hidden !text-transparent selection:bg-transparent"
                ...
                <div
                    ...
                    <template x-if="!config.multiselect">
                        <div @class([
                            ...
                            <span class="truncate text-sm text-secondary-700 dark:text-secondary-400"
                    ...
                    <template x-if="config.multiselect">
                        <div class="absolute inset-y-0 left-0 flex w-full items-center overflow-hidden pl-3 pr-14"
                            x-on:click="toggle">
                            <div class="flex items-center gap-2 overflow-x-auto hide-scrollbar">
                                ...
                                <div wire:ignore class="flex flex-nowrap items-center gap-1">
                                    <template x-for="(option, index) in selectedOptions" :key="`selected.${index}`">
                                        <span
                                            class="inline-flex items-center rounded-full border border-secondary-200 bg-secondary-100 py-0.5 pl-2 pr-0.5 text-xs font-medium text-secondary-700 shadow-sm dark:border-none dark:bg-secondary-700 dark:text-secondary-400">
                                            <span style="max-width: 5rem" class="truncate" x-text="option.label"></span>

                                            <button
                                                class="flex h-4 w-4 shrink-0 items-center justify-center text-secondary-400 hover:text-secondary-500"
                    ...
                    @if ($clearable && !$readonly && !$disabled)
                        <button x-show="!isEmpty()" x-on:click="clear" tabindex="-1" type="button" x-cloak>
                            <x-dynamic-component :component="WireUi::component('icon')"
                                class="h-4 w-4 text-secondary-400 hover:text-negative-400" name="x" />
                        </button>
                    @endif
    ...
    <x-wireui::parts.popover :margin="(bool) $label" class="sm:max-w-xs">
        <template x-if="asyncData.api || (config.searchable && options.length >= @toJs($minItemsForSearch))">
            <div class="my-2 px-2" wire:key="search.options.{{ $name }}">
        ...
        <div class="max-h-64 select-none overflow-y-auto overscroll-contain soft-scrollbar sm:max-h-60" tabindex="-1"
            x-ref="optionsContainer" name="wireui.select.options.{{ $name }}"
            x-on:keydown.tab.prevent="$event.shiftKey || getNextFocusable().focus()"
            x-on:keydown.arrow-down.prevent="$event.shiftKey || getNextFocusable().focus()"
            x-on:keydown.shift.tab.prevent="getPrevFocusable().focus()"
            x-on:keydown.arrow-up.prevent="getPrevFocusable().focus()">
            <div class="relative h-0.5 w-full overflow-hidden rounded-full"
                :class="{
                    'bg-gray-200 dark:bg-gray-700': asyncData.fetching
                }">
                <div class="absolute h-0.5 animate-linear-progress rounded-full bg-primary-500" style="width: 30%"
                ...
                <template x-for="(option, index) in displayOptions" :key="`${index}.${option.value}`">
                    <li tabindex="-1" :index="index">
                        <div class="px-2 py-0.5">
                            <div class="h-8 w-full animate-pulse rounded bg-slate-200 dark:bg-slate-600"></div>
                        </div>
                    </li>
                </template>
            </ul>
            ...
            @unless ($hideEmptyMessage)
                <div class="cursor-pointer px-3 py-12 text-center text-secondary-500 sm:px-3 sm:py-2 sm:text-left"

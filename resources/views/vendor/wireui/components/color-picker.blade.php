<div x-data="wireui_color_picker({
    ...
    @if ($attributes->wire('model')->value()) wireModifiers: @toJs($attributes->wireModifiers()),
        wireModel: @entangleable($attributes->wire('model')), @endif
            ...
            <div class="absolute inset-y-0 right-0 flex items-center p-0.5">
                <x-dynamic-component :component="WireUi::component('button')" class="h-full rounded-r-md" primary flat squared
                    ...
                    <x-dynamic-component :component="WireUi::component('icon')"
                        class="h-4 w-4 text-gray-400 group-focus:text-primary-700 dark:text-gray-600 dark:group-hover:text-gray-500 dark:group-focus:text-primary-500"
                        :name="$rightIcon" />
                </x-dynamic-component>
            </div>
        </x-slot>
    ...
    <x-wireui::parts.popover :margin="(bool) $label" @class(['max-h-64 select-none overflow-hidden', 'sm:max-w-[19rem]'])>
        <div @class([
            ...
            <template x-for="(color, index) in colors" :key="index">
                <button @class([
                    ...
                    x-on:click="select(color)" :title="color.name" type="button">
                </button>
            </template>
        </div>

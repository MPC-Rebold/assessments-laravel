<div>
    <div class="flex items-center">
        @if ($leftLabel)
            <x-dynamic-component :component="WireUi::component('label')" :for="$id" class="mr-2" :label="$leftLabel"
        ...
        @if ($label)
            <x-dynamic-component :component="WireUi::component('label')" :for="$id" class="ml-2" :label="$label"
                :has-error="$errors->has($name)" />
        @endif
    </div>
